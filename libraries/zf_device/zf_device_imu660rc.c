...
//-------------------------------------------------------------------------------------------------------------------
// 函数简介     获取 IMU660RC 设备 ID
// 参数说明     void
// 返回参数     uint8           正常应为 0x70
// 使用示例     uint8 chip_id = imu660rc_get_chip_id();
// 备注信息     用于通信异常后的 SPI 链路诊断
//-------------------------------------------------------------------------------------------------------------------
uint8 imu660rc_get_chip_id (void)
{
    return imu660rc_read_register(IMU660RC_CHIP_ID);
}
