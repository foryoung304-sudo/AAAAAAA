...
## 2026-07-24 实飞确认 PID 基线

- 状态：重新编译、下载并实飞后，用户确认飞行恢复正常。
- 恢复依据：审计 2026-07-17 至 2026-07-21 的附件日志后，采用 2026-07-18 附件 `98621878-4ae5-403a-8f1c-bfd45197131b` 中完整的 `FLIGHTCFG`。后续任务固件继续沿用该组 Flash PID，修改的是终端速度包络和阻尼结构。
- 文档前面的旧“姿态环测试”参数仅保留为历史记录，不得用于当前恢复。

```text
ANGLE R/P/Y: 3.190 / 3.184 / 0.872
RATE R:      0.1228 / 0.0260 / 0.0024
RATE P:      0.1353 / 0.0300 / 0.0020
RATE Y:      0.3540 / 0.0250 / 0.0003

POS X:       0.351 / 0 / 0
POS Y:       0.311 / 0 / 0
VEL X:       1.1120 / 0.2435 / 0.0510
VEL Y:       1.2081 / 0.2300 / 0.0605
...
- `FLIGHTCFG` 不记录 Angle I/D 或 ALT H；Angle I/D 的零值及 ALT H 来自同期源码。
- `FLIGHTCFG` 末尾的 `8.000 / 50.000` 是 LOC 倾角上限与水平速度上限，不是高度 PID。
- PID Flash 当前版本为 v4。旧 v1/v2/v3 Flash 不得覆盖上述源码基线；确认菜单参数一致后，长按 K4 保存为 v4。
- 出现“代码没改但飞行表现突变”时，先核对 Rebuild、测试/任务模式、Flash 版本和同趟日志末尾 `FLIGHTCFG`，不要直接重新调 PID。
- 当前任务模式为 `VISION_MISSION_MODE_ENABLE=1`，任务调试输出为 `MISSION_DEBUG_LOG_ENABLE=1`。

### 2026-07-24 任务首飞高度余量

- 预检第2次仅因ToF尚未出帧而被`flags=0x02`阻止，约4.2秒后第3次预检全部通过，不属于传感器故障。
- 任务飞行中高度长期约`89~94 cm`，未达到`135 cm`巡航就绪门槛；电压由解锁约`11.89 V`降至约`10.2 V`。
- 当前电压补偿按`gain=4.0`计算需要约`6.8`个百分点，但原`ALT_HOVER_TRIM_MAX=6.0`已截断。因此仅将上限保守提高到`7.5`，增益`4.0`和渐入速度`1.0/s`不变；PID及悬停基准不变。
- 下一趟只验证能否平稳继续爬升并越过`135 cm`。若出现明显快速上冲，立即把`ALT_HOVER_TRIM_MAX`恢复为`6.0`，不得同时调整高度PID。
